...
class RecursiveSolution {
    ...
    reverseList(head) {
      ...
      let newHead = head; // maintain the current head
      if (head.next) { // if there is still a sub-problem, if we can keep reversing
        ...
        head.next.next = head; // reverse the link between the next node and head
      }
      head.next = null; // if head happens to be the first node in the list, we're setting the next pointer to null, because since it's reversed now, it'll be the last item in the list and it won't have a next
      return newHead;
    }
}